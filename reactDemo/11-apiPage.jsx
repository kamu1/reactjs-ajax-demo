...
var MainBox=React.createClass({
	...
	requestDataList:function(config){
		...
		}else if (!config.data.pageSize){
			config.data.pageSize=3;
		}
	...
	delMultiselect:function(){
		...
		if (window.confirm("确定要删除选中的数据吗?")){
			...
			this.props.ajax(config, function(){
				...
				this.props.requestDataList(config);
			}.bind(this));
...
var Page = React.createClass({
	onClick:function(event, num){
		if (this.props.page.pageNumber != num){
			var config = {
				url:"api/index.php",
				data: {
					pageNumber:num
	...
	render:function(){
		...
		if (pageNum == page.pageNumber){
			stylePage.border="1px solid transparent";
			stylePage.color="#999";
			stylePage.cursor="default";
			stylePage.background="none";
			
		}
		return (
			<a style={stylePage} onClick={(ev, num) => {this.onClick(ev, pageNum)}}>{pageNum}</a>
		)
	}
});
